...
async def test_activities_cached(async_client: AsyncClient, test_user, monkeypatch):
    ...
    assert resp1.status_code == 200
    assert resp1.json() == [{"id": 1, "name": "Run"}]
    assert calls["count"] == 1
    ...
    assert resp2.status_code == 200
    assert resp2.json() == [{"id": 1, "name": "Run"}]
    assert calls["count"] == 1


@pytest.mark.asyncio
async def test_activity_details_cached(async_client: AsyncClient, test_user, monkeypatch):
    data = {"username": test_user.email, "password": "admin"}
    token_resp = await async_client.post("/auth/token", data=data)
    token = token_resp.json()["access_token"]
    headers = {"Authorization": f"Bearer {token}"}
    ...
    def fake_fetch(activity_id, token, include_all_efforts=True):
        calls["count"] += 1
        return {"id": activity_id, "detail": "live"}

    monkeypatch.setattr(strava_router, "fetch_activity_details", fake_fetch)

    resp1 = await async_client.get("/strava/activities/1", headers=headers)
    assert resp1.status_code == 200
    assert resp1.json() == {"id": 1, "detail": "live"}
    assert calls["count"] == 1

    def fail_fetch(activity_id, token, include_all_efforts=True):
        raise RuntimeError("should not fetch again")

    monkeypatch.setattr(strava_router, "fetch_activity_details", fail_fetch)

    resp2 = await async_client.get("/strava/activities/1", headers=headers)
    assert resp2.status_code == 200
    assert resp2.json() == {"id": 1, "detail": "live"}
    assert calls["count"] == 1
