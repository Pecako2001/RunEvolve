...
describe("CreateRunPage", () => {
  ...
  test("renders basic elements like the title", () => {
    ...
    expect(
      screen.getByRole("heading", { name: /Create New Run/i, level: 1 }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: /Preview Previous Run/i, level: 2 }),
    ).toBeInTheDocument();
  });
  ...
  describe("Display Previous Run Data", () => {
    test("shows loading indicator initially", () => {
      ...
      render(<CreateRunPage />);
      expect(
        screen.getByText(/Loading previous run details.../i),
      ).toBeInTheDocument();
    });
    ...
    test("displays error message on API failure", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/Error fetching last run/i),
        ).toBeInTheDocument();
    ...
    test('displays "no data" message on 404 API response', async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText(/No previous run data available to preview./i),
        ).toBeInTheDocument();
      });
    });
  });
