...
export default function StatisticsPage() {
  ...
  const [errorCompletedRuns, setErrorCompletedRuns] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/runs`) // This endpoint now returns completed runs
      .then(async (response) => {
        if (!response.ok) {
          const errorData = await response
            .json()
            .catch(() => ({ detail: "Failed to fetch completed runs data." }));
          throw new Error(
      ...
      .catch((err) => {
        ...
        setErrorCompletedRuns(
          err.message ||
            "An unexpected error occurred while fetching completed run data.",
        );
        setCompletedRuns([]);
      })
      .finally(() => {
    ...
    fetch(`${API_BASE_URL}/runs/planned`)
      .then(async (response) => {
        if (!response.ok) {
          const errorData = await response
            .json()
            .catch(() => ({ detail: "Failed to fetch planned runs" }));
          throw new Error(
            errorData.detail || `HTTP error! status: ${response.status}`,
          );
        }
  ...
  const monthlyAvgSpeedData = completedRuns
    .reduce(
      (acc, run) => {
        if (
          !run.created_at ||
          run.average_speed === null ||
          run.average_speed === undefined
        )
          return acc;
        ...
        if (existingEntry) {
          ...
          existingEntry.avgSpeed =
            existingEntry.totalSpeed / existingEntry.count;
        } else {
          acc.push({
            month: monthYear,
    ...
    return (
      <Container fluid className={styles.statisticsPage__container}>
        <Stack>
          {errorCompletedRuns && (
            <Alert
              title="Error Fetching Completed Runs"
              color="red"
          ...
          {errorPlannedRuns && (
            <Alert
              title="Error Fetching Planned Runs"
              color="red"
              icon={<IconAlertCircle />}
            >
              {errorPlannedRuns}
  ...
  if (
    ...
    return (
      <Container fluid className={styles.statisticsPage__container}>
        <Title order={2} className={styles.statisticsPage__mainTitle}>
          Run Statistics
        </Title>
        <Text>
          No run data available. Create some completed or planned runs first!
        </Text>
      </Container>
    );
  }
  ...
  const plannedTableRows = plannedRuns.map((run) => (
    <Table.Tr key={run.id}>
      ...
      <Table.Td>
        {run.settings_snapshot?.notes ||
          run.settings_snapshot?.predicted_run_type ||
          "N/A"}
      </Table.Td>
    </Table.Tr>
  ));
    ...
    <Container fluid className={styles.statisticsPage__container}>
      <Stack className={styles.statisticsPage__contentStack}>
        ...
        <Grid>
          <Grid.Col span={{ base: 12, md: 6 }}>
            <Paper className={styles.statisticsPage__chartPaper}>
              <Title order={4} className={styles.statisticsPage__paperTitle}>
        ...
        <Paper className={styles.statisticsPage__tablePaper}>
          ...
          {!loadingPlannedRuns &&
            !errorPlannedRuns &&
            plannedRuns.length === 0 && <Text>No planned runs yet.</Text>}
          {!loadingPlannedRuns &&
            !errorPlannedRuns &&
            plannedRuns.length > 0 && (
              <ScrollArea>
