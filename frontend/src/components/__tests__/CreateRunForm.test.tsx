...
describe("CreateRunForm", () => {
  ...
  test("renders the form with input and button", () => {
    ...
    expect(
      screen.getByRole("button", { name: /Create Run From Last/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Note: The run name is currently for local reference/i),
    ).toBeInTheDocument();
  });
  ...
  describe("Button Click and API Interaction", () => {
    test("calls API, shows loading state, and success notification", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/New run created successfully!/i),
        ).toBeInTheDocument();
